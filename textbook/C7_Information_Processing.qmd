...
date: 09/01/2021
abstract: "This chapter covers notions of *processing stages*, *information*, and *capacity limitations*, which became popular research topics around the 1950s and 60s."
license: 'CC BY-SA'
...
For example, how does an impulse surpass a threshold and trigger a reflex, what is the mechanism? In Skinner's system this question could be viewed as incoherent by definition. He defined those terms not to have any reference to physical mechanisms. Of course, it is possible to wonder about what physiological mechanisms do give rise to the behaviors in Skinner's descriptive system, but the investigation of such mechanisms was not in the same domain as behaviorism.
...
@fig-7Donders_simple shows the most basic *simple reaction time task*. This task measures how long it takes for a person to detect that a stimulus has been presented. Participants would wait for a stimulus, like a light, to be presented and then they would make a response as quickly as possible to, as soon as the stimulus was detected.
Donders would consider performance in this task as measure of "physiological reaction time".
...
Donders invented the idea of having subjects perform a slightly more complicated task shown in @fig-7Donders_GONoGo, now referred to as the *GO-NO GO* task. Participants are presented with a stimulus, but they are instructed to only make a response when the stimulus is the target "go" stimulus. For example, one might see a blue or red stimulus, and the response would be required only if the stimulus was blue (a GO response). No response would be required if the stimulus was red (a NO GO trial).
...
@fig-7Donders_Choice depicts a more complicated *alternative forced-choice task*. Here, Participants are shown one of many stimuli and are required to respond to each with a specific response. For example, a subject could be asked to respond to a blue stimulus by pressing a left button, and to respond to a red stimulus by pressing a right button. This would be called a 2-AFC (two-alternative forced choice) task.

### Donders subtractive stage logic

Donders used the subtraction of reaction times between tasks to estimate the speed of specific mental operations. He assumed that mental operations occurred in successive stages like an assembly line (shown in @fig-7Donders_Assembly), and that each stage took an average amount time. 
...
According to Donders, the amount of time taken to respond to a stimulus should depend on how many processing stages were required for the task at hand. More simple tasks have fewer processing stages and can be performed more quickly than more complicated tasks that have more required processing stages. These ideas are illustrated in @fig-7Donders_task_stages.
...
The fastest reaction time should be the physiological time given by the simple reaction time task. Reaction times should increase in length if additional mental processing had to occur before a response was made. 
...
For example, if it took 170 milliseconds to make a response in the GO-NO GO task, and 150 milliseconds to make a response in the simple reaction time task, then Donders took the difference of 20 milliseconds (by subtraction 170-150 = 20) to indicate stimulus identification time.

Subtraction logic could be applied to infer the times associated with subsequent stages of processing. For example, reaction times in the choice task (2AFC) are longer than in a GO-NO GO task. Following Donders logic, a 2AFC task involves yet another mental operation, *response selection*. For example, in a choice task a stimulus must be identified, and then the correct response (e.g., right or left) must be selected before the final response is made.
...
What happened to Donders subtraction logic, research on human reaction times, and the concept of processing stages since he proposed them in 1865?
...
Bits measure the total number of discrete events in a system of messages. Let's see how.

A single bit has two states, 0 or 1. So, we could use a single bit to represent two unique states, like heads or tails.

How many states can two bits represent? This would involve counting all of the unique ways of combining the states from two bits, for example, the first and second bit could be 0 (00), the first bit could be zero and the second a one (01), and so on. All of four possibilities are: 00, 01, 10, and 11. @fig-7Shannon_bits shows the relationship between number of bits, and the number of unique events that can be represented by combining bits together. 
...
@fig-7Hick_hyman_law shows the results for two (of four) subjects reported by Hyman. Hyman had conducted a third experiment where he manipulated the amount of information separately from the number of alternatives in a slightly different way; and these results are also included in the plot. 
...
**Note for PSYC 2040**: Before we jump into memory, we will take a detour into the idea of computational models and their relevance to the study of cognition. We will return to this textbook after the "Cognitive Models" learning module.
