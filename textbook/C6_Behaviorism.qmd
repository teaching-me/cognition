...
date: 09/01/2021
abstract: "This chapter covers the school of behaviorism, which rose to prominence in American psychology in the early 20th century."
license: 'CC BY-SA'
...
The previous chapters described characters in the historical play of psychology that set the stage for the entry of behaviorism. Already on stage were the "mentalists" and the "physicalists". The mentalists were interested in examining so-called "mental" processes of cognition. For example, Galton assumed that subjective experiences of mental images was a real thing that differed among people, and that could be an object of scientific inquiry. Titchener had developed introspectionism, where people could be trained to carefully, systematically, and "accurately" describe their inner mental experiences; and, those experiences were regarded as worthy of scientific inquiry. Even Thorndike entertained the notion that animals experience mental simulations (e.g., about the possibility of food reward following escape from a puzzle box) that link associations between a stimulus situation and an eventual reward. The "physicalists" were interested in examining and understanding the physiological basis of psychological processes. For example, Pavlov's goal was to measure how stimulus-response learning processes were reflected in the production of glandular secretions. In the play, both sides criticized each other on several grounds-- the mentalists lacked objective measures and definitions, the physiologists were too reductive and weren't studying cognition. 

A few additional backdrops framed the entry of behaviorism. One backdrop was scientific credentialism, which involves claims about the credentials a field of study must possess in order to be considered a "true" science. Psychologists were interested in gaining credentials so that it would be recognized by society and other sciences as a proper science in its own right. The concern that psychology should become a science is related to the backdrops of [positivism](https://en.wikipedia.org/wiki/Positivism), and scientific [utopianism](https://en.wikipedia.org/wiki/Utopia#Utopianism). There are many philosophies of science that define the credentials of science in different ways, and prominent strains of behaviorism were constructed in the positivist tradition. Scientific utopianism refers broadly to the notion that society can be improved through science and technology. Positivism and scientific utopianism are discussed in turn before proceeding to the main act...behaviorism.

### Positivism

```{r}
#| label: fig-6Comte
#| fig-cap: "August Comte (1798-1857)"
#| column: margin
...
In the remaining sections, instances of behaviorism are explored further by examining prominent individuals and their individual flavors of behaviorism. There were/are many behaviorists, but this chapter limits discussion to [J. B. Watson](https://en.wikipedia.org/wiki/John_B._Watson), [Edward Tolman](https://en.wikipedia.org/wiki/Edward_C._Tolman) (1886–1959), [Clark L. Hull](https://en.wikipedia.org/wiki/Clark_L._Hull) (1884-1952) and [Burrhus F. Skinner](https://en.wikipedia.org/wiki/B._F._Skinner) (1904–1990).

## Watson's "Salesman" Behaviorism

```{r}
#| label: fig-6Watson
#| fig-cap: "John Broadus Watson (1878-1958)."
#| column: margin
...
In positivism, a scientific system is achieved when a phenomena under investigation can be described and predictably controlled. Watson envisioned stimulus-response learning as a positivist system. Elements of Watson's S-R system are described below with figures showing original text from @watsonBehaviorism1924's book.

```{r}
#| label: fig-6Watson_SR
#| fig-cap: "Watson's S-R formula."
#| fig-width: .5

knitr::include_graphics("imgs/Watson_SR.png")
```

In @fig-6Watson_SR, Watson stated the problem for behaviorism as finding the functional relationship between stimuli and responses, such that one could be predicted or ascertained from the other. He started with simple examples of S-R associations, and then claimed the concept could be generalized to much wider applications in society.

```{r}
#| label: fig-6Watson_UCR
#| fig-cap: "Watson describing unconditioned responses."

knitr::include_graphics("imgs/Watson_unconditioned.png")
```

In @fig-6Watson_UCR, Watson lists simple S-R associations, like the patellar knee reflex, that do not require formation through learning.

```{r}
#| label: fig-6Watson_CR
#| fig-cap: "Watson describing how a conditioning process could change a child's reaction to a dog."

knitr::include_graphics("imgs/Watson_conditioned.png")
```

In @fig-6Watson_CR, Watson suggests that Pavlovian conditioning  could form new S-R associations through experience. He presented conditioning as a very general process that could cause wide ranges of behaviors to come under stimulus control.
...
```{r}
#| label: fig-6Watson_Society_SR
#| fig-cap: "Watson considers how his S-R system could be applied to societal issues. "
#| fig-subcap: 
#|   - "What responses follow from these stimuli?"
#|   - "What stimuli are needed to bring about these responses?" 
#| layout-ncol: 2
...
In @fig-6Watson_Society_SR, Watson formulated societal level manipulations in terms of stimulus-response questions, and was hopeful that behaviorism could help make predictions about outcomes of such interventions. Similarly, he imagined desired outcomes for society, and implied that behavioral engineering would be able to construct the appropriate stimuli to force the behaviors he considered desirable.

```{r}
#| label: fig-6Watson_sweep
#| fig-cap: "Watson envisions how behaviorism will envelope existing social institutions."
#| column: margin

knitr::include_graphics("imgs/Watson_sweep.png")
```

Finally, Watson promoted behaviorism as more than the correct scientific way to do psychology. It was an academic social movement in the tradition of scientific utopias that was sweeping the nation. For example, in @fig-6Watson_sweep Watson describes how psychology, philosophy, ethics, social psychology, sociology, religion, and psycho-analysis were all becoming behaviorist systems or disappearing if they were not.
...
```{r}
#| label: fig-6Tolman
#| fig-cap: "Edward C. Tolman (1886-1959)"
#| column: margin
...
```{r}
#| label: fig-6Tolman_AlleyMaze
#| fig-cap: "Depiction of an alley maze used by Tolman."
#| column: margin

knitr::include_graphics("imgs/Tolman_AlleyMaze.png")
```

Tolman argued that behaviors were obviously goal-driven and should be described in terms of goals and purposes. To support this argument, he showed that maze running behavior could be influenced by apparent goals or motivational drives. @fig-6Tolman_AlleyMaze depicts one of Tolman's "Alley Mazes". The maze had a start point where the rat entered, and an end point where it could receive food reward. In between, the rat had to navigate the maze, which contained several corridors, doors, and curtains, and many "blind-alleys" leading to an empty wall. In general, rats explore the maze and eventually find the food reward. The behavior of the rat can be measured in different ways, such as the total amount of time taken to successfully navigate the maze. In the following experiment, Tolman measured the number of errors an animal made during navigation. An error is counted whenever the rat enters a blind-alley entrance.


In one experiment [@tolmanDegreesHungerReward1930], Tolman manipulated level of hunger and amount of reward. Rats ran the maze when they were either very hungry or not very hungry. Additionally, for both groups, food was either given as a reward at the end of the maze, or food was not given. 

```{r}
#| label: fig-6Tolman_hunger
#| fig-cap: "Results from @tolmanDegreesHungerReward1930."
#| out-width: "50%"

knitr::include_graphics("imgs/Tolman_hunger.png")
```

The results of the experiment are shown in @fig-6Tolman_hunger. In general, all groups of rats showed fewer errors across the days of the experiment:  on day one, the lines started with a high number of errors, and trend downward over days to a lower number of errors. This means that the rats made fewer wrong turns as they gained experience with navigating the maze over days. 
...
```{r}
#| label: fig-6Hull
#| fig-cap: "Clark Hull (1884-1952)"
#| column: margin

knitr::include_graphics("imgs/clark-l-hull.jpg")
```

[Clark L. Hull's](https://en.wikipedia.org/wiki/Clark_L._Hull) (1884-1952) ^[Hull also worked during the eugenics era in American psycholgoy. He was recruited to Yale in 1929 by then president James R. Angell, who was previously a psychologist running a nationally recognized eugenics laboratory. Hull was recruited into Yale's burgeoning new enterprise, the Institute for Human Relations (IHR). Hull was hired as a mental tester, and among his first grant applications to the institute was a program of eugenics (see also the role of eugenics in the early Yale psychology department [@doyleMeasuringProblemsHuman2014] and more on Hull's influence in the IHR [@morawskiOrganizingKnowledgeBehavior1986])] flavor of behaviorism was again in the positivist tradition, but more focused on using math to specify functional relationships between units of behavior. In the positive tradition, a scientific theory is tentative and pragmatic. It is useful if it works for purposes of manipulation and control. Mathematical formulations are not strictly necessary, but they are often useful in describing functional relationships for purposes of prediction. 
...
```{r}
#| label: fig-6Skinner
#| fig-cap: "Burrhus F. Skinner (1904–1990)"
#| column: margin
...
```{r}
#| label: fig-6SkinnerBox
#| fig-cap: "Depiction of a Skinner box."
#| column: margin

knitr::include_graphics("imgs/Skinner_Box.png")
```

@fig-6SkinnerBox shows the Skinner box had space for a single rat to move about, and mechanisms for water and food delivery. The box also contained a lever the animal could press in order to receive food reward (subject to experimenter control). And, the box was set up to record any lever presses the animal made.
...
```{r}
#| label: fig-6SkinnerSlope
#| fig-cap: "Depiction of how a low response rate over time yields a shallow slope (red line), and a high response rate over time yields a steep slope (aqua line)."
#| out.width: "75%"
...
```{r}
#| label: fig-6Skinnerdescription
#| fig-cap: "Depiction of Skinner's goal of systematically describing level-pressing behavior over time."
#| out.width: "75%"

knitr::include_graphics("imgs/Skinner_description.png")
```

The graph in @fig-6Skinnerdescription is purely descriptive in two senses. First, the line describes lever-pressing over conditions and phases of the experiment. Second, the letters for individual line segments highlight how Skinner's system involved using abstract terms to describe what he argued were generalizable components of behavior. To contrast some ways we could describe this graph, first let me describe it using "everyday language", then let's use Skinner's system to describe it. 
